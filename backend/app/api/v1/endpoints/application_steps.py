...
def _build_step_response(
    ...
    completed_sections = app.form_metadata.get(
        "completed_sections", []) if app.form_metadata else []
    next_step = None
...
@router.patch("/{application_id}/steps/1/personal-details",
    ...
    **Step 1/12: Personal Details**
    ...
    try:
        app = app_service.update_personal_details(
            ...
            # Use mode='json' to serialize dates/datetimes
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN if isinstance(
                e, ApplicationPermissionError) else status.HTTP_404_NOT_FOUND if isinstance(
                e, ApplicationNotFoundError) else status.HTTP_400_BAD_REQUEST, detail=str(e))
...
@router.patch("/{application_id}/steps/2/emergency-contact",
              response_model=StepUpdateResponse)
async def update_emergency_contact(
    ...
    **Step 2/12: Emergency Contact**
    ...
    try:
        ...
        return _build_step_response(
            app, 2, "emergency_contact", f"{
                len(contacts)} emergency contact(s) saved")
...
@router.patch("/{application_id}/steps/3/health-cover",
              response_model=StepUpdateResponse)
async def update_health_cover(
    ...
    **Step 3/12: Health Cover (OSHC)**
    ...
    try:
        ...
        return _build_step_response(
            app, 3, "health_cover", "Health cover details saved")
...
@router.patch("/{application_id}/steps/4/language-cultural",
              response_model=StepUpdateResponse)
    ...
    **Step 4/12: Language & Cultural Background**
    ...
    try:
        ...
        return _build_step_response(
            ...
            4,
            "language_cultural",
            "Language and cultural details saved")
...
@router.patch("/{application_id}/steps/5/disability-support",
              response_model=StepUpdateResponse)
async def update_disability_support(
    ...
    **Step 5/12: Disability Support**
    ...
    try:
        ...
        return _build_step_response(
            app, 5, "disability", "Disability support details saved")
...
@router.patch("/{application_id}/steps/6/schooling-history",
              response_model=StepUpdateResponse)
async def update_schooling_history(
    ...
    **Step 6/12: Schooling History**
    ...
    try:
        ...
        return _build_step_response(
            app, 6, "schooling", f"{
                len(entries)} education entries saved")
...
@router.patch("/{application_id}/steps/7/qualifications",
              response_model=StepUpdateResponse)
async def update_qualifications(
    ...
    **Step 7/12: Previous Qualifications**
    ...
    try:
        ...
        return _build_step_response(
            app, 7, "previous_qualifications", f"{
                len(entries)} qualification(s) saved")
...
@router.patch("/{application_id}/steps/8/employment-history",
              response_model=StepUpdateResponse)
async def update_employment_history(
    ...
    **Step 8/12: Employment History**
    ...
    try:
        ...
        return _build_step_response(
            app, 8, "employment", f"{
                len(entries)} employment entries saved")
...
@router.patch("/{application_id}/steps/9/usi",
              response_model=StepUpdateResponse)
...
@router.patch("/{application_id}/steps/10/additional-services",
              response_model=StepUpdateResponse)
    ...
    **Step 10/12: Additional Services**
    ...
    try:
        ...
        return _build_step_response(
            ...
            10,
            "additional_services",
            f"{selected_count} service(s) selected")
...
@router.patch("/{application_id}/steps/11/survey",
              response_model=StepUpdateResponse)
async def update_survey(
    ...
    **Step 11/12: Survey**
    ...
    try:
        ...
        return _build_step_response(
            app, 11, "survey", "Survey responses saved")
...
@router.get("/{application_id}/steps/12/documents",
            response_model=DocumentStepResponse)
async def get_document_status(
    ...
    **Step 12/12: Document Upload Status**
    ...
    """
    from app.models import ApplicationStage, Document, DocumentType
    ...
    try:
        ...
        current_stage_index = stage_order.index(
            app.current_stage) if app.current_stage in stage_order else 0
        ...
        for doc_type in document_types:
            uploaded_doc = next(
                (d for d in uploaded_docs if d.document_type_id == doc_type.id), None)
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN if isinstance(
            e, ApplicationPermissionError) else status.HTTP_404_NOT_FOUND, detail=str(e))
