...
class RtoProfile(Base):
    ...
    brand_settings = Column(JSONB, nullable=True)
    # {default_commission_rate, sla_overrides, features_enabled}
    business_settings = Column(JSONB, nullable=True)
...
class UserAccount(Base):
    ...
    # JSONB fields
    # {email: {enabled, frequency}, sms: {...}, in_app: {...}}
    notification_preferences = Column(JSONB, nullable=True)
    # For staff/admin: workflow_sla, default_templates
    admin_config = Column(JSONB, nullable=True)
...
class AgentProfile(Base):
    ...
    commission_rate = Column(
        Numeric(5, 2), nullable=True)  # e.g., 15.00 for 15%
...
class StaffProfile(Base):
    ...
    # JSONB field
    # {can_approve_applications, can_manage_users, ...}
...
class Application(Base):
    ...
    current_stage = Column(
        ...
        nullable=False,
        default=ApplicationStage.DRAFT,
        index=True)
    ...
    health_cover_policy = Column(JSONB, nullable=True)
    # {has_disability, disability_details, support_required, documentation_status}
    ...
    additional_services = Column(JSONB, nullable=True)
    # {interview_date, staff_id, scorecard, decision, notes}
...
Index(
    "idx_application_enrollment",
    Application.enrollment_data,
    postgresql_using="gin")
Index(
    "idx_application_emergency",
    Application.emergency_contacts,
    postgresql_using="gin")
Index(
    "idx_application_health_cover",
    Application.health_cover_policy,
    postgresql_using="gin")
Index(
    "idx_application_disability",
    Application.disability_support,
    postgresql_using="gin")
Index(
    "idx_application_language",
    Application.language_cultural_data,
    postgresql_using="gin")
Index(
    "idx_application_survey",
    Application.survey_responses,
    postgresql_using="gin")
Index(
    "idx_application_services",
    Application.additional_services,
    postgresql_using="gin")
Index(
    "idx_application_gs_assessment",
    Application.gs_assessment,
    postgresql_using="gin")
Index(
    "idx_application_signatures",
...
class ApplicationStageHistory(Base):
    ...
    def __repr__(self):
        return f"<ApplicationStageHistory(app={
            self.application_id}, {
            self.from_stage} → {
            self.to_stage})>"
...
class DocumentType(Base):
    ...
    name = Column(String(255), nullable=False)
    # Which stage requires this document
...
class Document(Base):
    ...
    # JSONB field
    # [{requested_by, requested_at, due_at, status}]
    gs_document_requests = Column(JSONB, nullable=True)
    ...
    def __repr__(self):
        return f"<Document(id={
            self.id}, type={
            self.document_type.code if self.document_type else 'N/A'})>"


# GIN index for JSONB field in DOCUMENT
Index(
    "idx_document_gs_requests",
    Document.gs_document_requests,
    postgresql_using="gin")
...
class AuditLog(Base):
    ...
    # e.g., "application", "document", "user_account"
